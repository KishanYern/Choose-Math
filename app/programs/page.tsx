...
export const metadata: Metadata = {
  title: "Top Mathematics Programs",
  description:
    "Explore the top national mathematics programs — from MIT and Princeton to UCLA and Carnegie Mellon — filtered by pure math, applied math, and statistics.",
};
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="border-b border-slate-800 bg-slate-900/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <p className="text-indigo-400 text-xs font-medium uppercase tracking-widest mb-3">Programs</p>
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-100 mb-3">
            Top Mathematics Programs
          </h1>
          <p className="text-slate-400 text-lg max-w-2xl leading-relaxed">
            The 10 most influential mathematics departments in the country — compare by focus area
          ...
          <div className="flex flex-wrap gap-4 mt-6 text-xs">
            {[
              { label: "Pure Math", color: "bg-indigo-500/15 text-indigo-400 border-indigo-500/20" },
              { label: "Applied Math", color: "bg-violet-500/15 text-violet-400 border-violet-500/20" },
              { label: "Statistics", color: "bg-cyan-500/15 text-cyan-400 border-cyan-500/20" },
            ].map((item) => (
      ...
      <div className="border-t border-slate-800 bg-slate-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <h2 className="text-xl font-semibold text-slate-100 mb-6">Choosing the Right Program</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={item.title} className="p-5 rounded-xl border border-slate-800 bg-slate-900/50">
                <h3 className="text-sm font-semibold text-slate-300 mb-2">{item.title}</h3>
