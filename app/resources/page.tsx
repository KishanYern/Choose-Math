    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="border-b border-slate-800 bg-slate-900/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          <p className="text-indigo-400 text-xs font-medium uppercase tracking-widest mb-3">Resources</p>
          ...
          <div className="flex flex-wrap gap-2 mt-5">
            ...
            <span className="text-xs px-2.5 py-1 rounded-full border border-slate-700 text-slate-500">
              = difficulty level
            </span>
          </div>
        </div>
        ...
        {byCategory.map(({ category, label, icon, items }) => (
          <section key={category}>
            ...
            <div className="flex items-center gap-3 mb-6">
              <span className="text-2xl math-symbol text-indigo-400 font-light">{icon}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {items.map((resource) => (
                <a
                  ...
                  className="group flex flex-col rounded-xl border border-slate-800 bg-slate-900/50 hover:border-indigo-500/40 hover:bg-slate-900 transition-all duration-200 overflow-hidden"
                >
                  <div className="p-5 flex-1">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {resource.free && (
                          <span className="text-xs px-1.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium">
                            Free
                          </span>
                        )}
                        <span
                  ...
                  <div className="px-5 pb-4 flex items-center justify-between gap-3">
                    <div className="flex flex-wrap gap-1">
                      {resource.tags.slice(0, 3).map((tag) => (
                        <span key={tag} className="text-xs px-1.5 py-0.5 rounded-md bg-slate-800 text-slate-600">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <span className="text-xs text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-0.5 font-medium flex-shrink-0">
                      Open
