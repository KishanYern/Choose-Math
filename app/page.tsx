...
const stats = [
  { value: "~$105K", label: "Median Starting Salary", sub: "BS Math, industry roles" },
  { value: "33%", label: "Employment Growth", sub: "Math occupations 2023–33 (BLS)" },
  { value: "7+", label: "Career Tracks", sub: "Finance, Tech, Academia & more" },
  { value: "Top 5", label: "Most Versatile Degree", sub: "Per job market analyses" },
];
...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden bg-slate-950 grid-bg">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-28 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <Link
              href="/careers"
              className="w-full sm:w-auto px-7 py-3.5 rounded-xl border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white font-medium text-base transition-colors"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-0 md:divide-x md:divide-slate-800">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center px-4">
                ...
                <p className="text-slate-300 text-sm font-medium mt-0.5">{stat.label}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {personas.map((persona) => (
            <Link
              key={persona.title}
              href={persona.href}
              className="group flex flex-col gap-4 p-7 rounded-2xl border border-slate-800 bg-slate-900/40 hover:border-indigo-500/40 hover:bg-slate-900 transition-all duration-200 card-hover"
            >
              <span className="text-4xl">{persona.icon}</span>
              <div>
                <h3 className="font-semibold text-slate-100 text-lg mb-2 group-hover:text-indigo-300 transition-colors">
                  {persona.title}
                </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { symbol: "Σ", title: "Top Compensation", desc: "Math graduates consistently rank among the highest-paid bachelor's degree holders." },
              ].map((item) => (
                <div key={item.title} className="p-5 rounded-xl border border-slate-800 bg-slate-900/50">
                  <span className="text-2xl math-symbol text-indigo-400 font-light">{item.symbol}</span>
                  <h3 className="text-slate-200 font-semibold text-sm mt-2 mb-1">{item.title}</h3>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-slate-400">Where math majors are building exceptional careers.</p>
          </div>
          <Link
            href="/careers"
            className="flex-shrink-0 text-sm text-indigo-400 hover:text-indigo-300 font-medium flex items-center gap-1 transition-colors"
          >
            All 7 paths
      ...
      {/* Career Fit Quiz CTA */}
      <section className="py-16 bg-gradient-to-b from-indigo-950/30 to-slate-950 border-y border-indigo-900/30">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <div className="text-5xl math-symbol text-indigo-400/40 mb-6 font-light">?</div>
          ...
          <p className="text-slate-600 text-sm mt-4">8 questions · ~2 minutes · No account needed</p>
        </div>
      </section>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-slate-400">Real mathematicians, real careers, real insight.</p>
          </div>
          <Link
            href="/stories"
            className="flex-shrink-0 text-sm text-indigo-400 hover:text-indigo-300 font-medium flex items-center gap-1 transition-colors"
      ...
      {/* FAQ */}
      <section className="py-20 border-t border-slate-800 bg-slate-900/20">
      ...
      {/* Final CTA */}
      <section className="py-24 px-4 sm:px-6 text-center bg-slate-950">
        <h2 className="text-3xl sm:text-4xl font-bold text-slate-100 mb-4">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          ...
          <Link
            href="/programs"
            className="w-full sm:w-auto px-7 py-3.5 rounded-xl border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white font-medium transition-colors"
          >
            Explore Programs
          </Link>
        </div>
